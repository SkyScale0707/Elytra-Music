...
|-------------|--------------------------|-----------------------------|-----------------------------------|
| **原版鞘翅**<br>Vanilla Elytra | 末地船战利品 / End ship loot | 无 / None | — |
...
| **去图腾化鞘翅**<br>Untotemized Elytra | 普通鞘翅触发图腾后自动转换<br>Automatically converted from a vanilla elytra after totem trigger | 触发时消耗大量耐久 / Trigger consumes large durability | 保留 **1/3 耐久（最低 18）**，**不可使用经验修补**；可用不死图腾在铁砧修复为满耐久原版鞘翅<br>Retains **1/3 durability (minimum 18)**, **cannot be repaired with Mending**; can be fully repaired back to a vanilla elytra using a Totem of Undying on an anvil |
...
- 需要 **Fabric Loader 0.19.3+** + **Fabric API**（Minecraft 1.20.1）
- 将模组 JAR 放入 `mods/` 文件夹
- Requires **Fabric Loader 0.19.3+** and **Fabric API** for Minecraft 1.20.1 – Place the mod JAR in your `mods/` folder
...
- **生存模式下**附魔限制生效（仅可附于原版鞘翅），创造模式会绕过此限制（原版行为）/ Enchantment restrictions apply in Survival (vanilla elytra only); Creative mode bypasses this (vanilla behavior)
